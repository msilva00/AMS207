...
# theta_samps = tail(theta_samps,0.6*num_iters)

# thin
theta_samps = theta_samps[seq(from = 1, to = length(mu_samps), by=5), ]
tau_samps = tau_samps[seq(from = 1, to = length(tau_samps), by=5)]
mu_samps = mu_samps[seq(from = 1, to = length(mu_samps), by=5)]

theta_summ = t(apply(theta_samps, 2, summary_stats))
theta_summ
colnames(theta_summ) = c("mean", "sd", "0.025", "0.975")
xtable(theta_summ)
...
colnames(summ_table) = c("mean", "sd", "0.025", "0.975")
xtable(summ_table)
