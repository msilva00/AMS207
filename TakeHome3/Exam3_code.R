...
for (i in 1:N) {
  alpha_post[i] = rbeta(1, I*sum(zi1_curr) + u_alpha, sum(zi2_curr) + v_alpha)
  ...
  gamma_post[i] = rgamma(1, (yi1 + yi2 +zi2_curr) + u_gamma, (lam_curr*ci*zi2_curr) + v_gamma)
...
plot.ts(tail(gamma_post,1000))
plot.ts(tail(lambda_post[1,], 1000))
plot.ts(zi1_post[3,])
